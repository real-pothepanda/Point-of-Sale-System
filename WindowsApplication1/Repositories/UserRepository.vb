...
Public Class UserRepository
    ...
    Public Sub Add(user As AppUser)
        Using connection = _factory.CreateConnection()
            connection.Open()
            Using command As New MySqlCommand("INSERT INTO users (full_name, username, password_hash, role, is_active) 
VALUES (@fullName, @username, @passwordHash, @role, @active)", connection)
    ...
    Public Sub Deactivate(id As Integer)
        Using connection = _factory.CreateConnection()
            connection.Open()
            Using command As New MySqlCommand("DELETE FROM users WHERE id = @id", connection)
